...
public abstract class CylinderParticleManager {
    ...
    public static void spawnParticlesAtPos(Level level, BlockPos pos, int particleCount, ParticleOptions particleType, ParticleDirection direction, double radius, double height, double yOffset) {
        if (!level.isClientSide) {
            ...
            for (int i = 0; i < particleCount; i++) {
                ...
                MagicManager.spawnParticles(level, particleType,
                        pos.getX() + .5f + xOffset, yPosition, pos.getZ() + .5f + zOffset,
                        0, directionVector.x, directionVector.y, directionVector.z, 0.1, true);
            }
        }
    }
